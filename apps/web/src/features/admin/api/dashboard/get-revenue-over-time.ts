...
import { and, asc, isNotNull, sql, sum } from "@virtbase/db";
...
export const getRevenueOverTime = cache(async () => {
  "use cache: private";
      ...
      async (tx) =>
        tx
          .select({
            ...
            amount: sql<number>`COALESCE(${sum(invoices.total)}, 0)::INTEGER`,
          })
          .from(
          ...
          .leftJoin(
            ...
            and(
              ...
              sql`${invoices.paidAt}::DATE = dtx.date`,
            ),
          )
