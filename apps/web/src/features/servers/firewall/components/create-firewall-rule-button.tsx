...
export function CreateFirewallRuleButton({
  disabled,
  onClick,
  size = "icon",
  variant = "outline",
  ...
  return (
    <>
      <Button
        ...
        size={size}
        onClick={(event) => {
          setAction("create-firewall-rule");
          onClick?.(event);
        }}
        disabled={action === "create-firewall-rule" || disabled}
        {...props}
      >
        {children}
      </Button>
      {action === "create-firewall-rule" && (
        <FirewallRuleDialog
          onOpenChange={(open) =>
            setAction(open ? "create-firewall-rule" : null)
          }
          open
          mode="create"
        />
