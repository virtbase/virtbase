...
type RegenerateCloudInitStepParams = {
  proxmoxNode: GetProxmoxInstanceParams;
  vmid: number;
};

export async function regenerateCloudInitStep({
  proxmoxNode,
  vmid,
}: RegenerateCloudInitStepParams) {
  "use step";

  const instance = getProxmoxInstance(proxmoxNode);
  await instance.node.qemu.$(vmid).cloudinit.$put();
}
