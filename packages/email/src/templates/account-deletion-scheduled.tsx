...
export default async function AccountDeletionScheduled({
  ...
  const t = createTranslator({
    ...
    namespace: "account-deletion-scheduled",
  });
        ...
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[600px] rounded border border-neutral-200 border-solid px-10 py-5">
            ...
            <Text className="mx-auto text-sm leading-6">
              {t("instructions", { appName: APP_NAME, date })}
            </Text>
            <Text className="text-black text-sm leading-6">{t("stop")}</Text>
            <Section className="my-8">
              <Link
                className="rounded-lg bg-black px-6 py-3 text-center font-semibold text-[12px] text-white no-underline"
                href={`${APP_DOMAIN}/account/settings/privacy`}
              >
                {t("cancel")}
              </Link>
            </Section>
